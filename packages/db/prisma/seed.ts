...
async function main() {
  ...
  // ─── SpecialRegimes ────────────────────────────────────────────────────────
  // regimeDefs/exitRuleDefs live in ./regime-data (shared with seed-regimes.ts).
  // Country is referenced by slug; resolve to countryId via countryMap below.
  const slugToCountryId = (slug: string) => {
    const id = countryMap.get(slug);
    if (!id) throw new Error(`Country slug "${slug}" not found for special regime`);
    return id;
  };
