...
router.post("/", async (req, res) => {
  ...
  const [homeCity, targetCity] = await Promise.all([
    prisma.city.findUnique({
      ...
      include: { col: true, translations: true, country: { select: { slug: true } } },
    }),
    prisma.city.findUnique({
  ...
  const homeCountrySlug = homeCity.country?.slug ?? "de";
  const homeTaxConfig = await prisma.homeTaxConfig.findUnique({
    where: { countrySlug: homeCountrySlug },
  });

  if (!homeTaxConfig) {
    res.status(500).json({ error: `Home tax config missing for country: ${homeCountrySlug}` });
    return;
  }
